...
public class MazeConstructor : MonoBehaviour
{
    ...
    public void GenerateNewMaze(int sizeRows, int sizeCols) //Size method called in GameController
    {
        ...
        for (int i = 0; i <= rMax; i++)        //Upper indices 
            for (int j = 0; j <= cMax; j++)            
                if (i == 0 || j == 0 || i == rMax || j == cMax)         //Checking for maze boundaries - must be wall       
                    maze[i, j] = 1;                                    
                else if (i % 2 == 0 && j % 2 == 0 && Random.value > placementThreshold)     //Whether non-bounrady spaces should be wall or corridor                                
                {
                    maze[i, j] = 1;
